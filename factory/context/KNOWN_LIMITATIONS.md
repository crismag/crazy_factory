...
- `crazy-admin run` closes the **process loop**. A raw `--prompt`
  ...
  `--prompt` on that product is a delta, not a recompile. No-key
  generic CRUD is **`RUNNABLE_PREVIEW`** (mechanical + runtime, not
  product COMPLETE). An owner delta re-opens work until VERIFIED
  against compiled claims; banner text is not evidence. A coding
  plugin is required for product-specific implementation.
  npm/browser journeys are still out of scope.
