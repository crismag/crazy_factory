...
- `crazy-admin run` closes the **process loop**. A raw `--prompt`
  ...
  `--prompt` on that product is a delta, not a recompile. The coding
  plugin still needs a live model to apply a non-banner change (the
  stdlib actuator will not clobber existing `src/app.py` on
  `implement`). npm/browser journeys are still out of scope.
