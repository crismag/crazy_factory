...
| ID | Task | Plan | Risk |
|---|---|---|---|
| **9E.ST6** | **Seed-derived ProjectContract** — derive structure/acceptance/forbidden-tech from the seed; supersede the hand-authored `architecture.json` (adds `data/tasks.json`, README, UI). **Also: decomposition must emit COHERENT, right-sized deliverables** (e.g. a module **+ its tests** as one item, or a vertical behavior slice), not per-file fragments — so each iteration accomplishes something observable (and the model+tests no longer split, removing the "no test in patch" rejection). Touches `completion.items_from_required_files`. **FOUNDATIONAL SLICE DONE** (`6ef44ae`): standalone `scripts/project_contract.py` — deterministic `parse_seed` / `derive_tech` (seed-grounded forbidden/allowed) / `contract_from_seed` / `validate_contract` / `to_architecture_contract` (renders the existing schema). 11 seed-grounded tests; **not yet wired** (no live-path change). **Remaining:** feed architect `modules`/`task_candidates` → `required_tree`; generate `architecture.json` from the seed at advance time; coherent right-sized decomposition; wire `required_behaviors`→acceptance + `forbidden_tech`→floor/adjudicator. | 02 | high |
